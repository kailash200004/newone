...
public class DailyPurchaseConfDaoImpl implements DailyPurchaseConfDao{
	...
	public List<DailyPurchaseConfModel> dpc2() {
		...
		querystr1 = "select (sum(x.value)/sum(x.netqty))as garsat, (sum(x.g1)/sum(x.netqty))as g11, (sum(x.g2)/sum(x.netqty)) as g22, (sum(x.g3)/sum(x.netqty))as g33, (sum(x.g4)/sum(x.netqty))as g44, (sum(x.g5)/sum(x.netqty))as g55, (sum(x.g6)/sum(x.netqty))as g66, (sum(x.g7)/sum(x.netqty)) as g77, (sum(x.g8)/sum(x.netqty)) as g88,x.datepurchase,x.cropyr,x.jutevariety,x.placeofpurchase,sum(x.netqty)as netqty\n"
				...
				+ "datepurchase, cropyr,jutevariety,placeofpurchase from jciprocurement \n"
				+ "where basis= 'commercial' \n"
				+ "group by grade1, grade2, grade3,grade4, grade5, grade6, grade7, grade8, netquantity,grasatrate,datepurchase,cropyr,jutevariety,placeofpurchase)x \n"
				+ " group by  x.jutevariety,x.datepurchase,x.cropyr,x.jutevariety,x.placeofpurchase;";
		...
		for(int i=0; i<query.list().size(); i++) {
			...
			SQLQuery query1 = session.createSQLQuery(querystr);
